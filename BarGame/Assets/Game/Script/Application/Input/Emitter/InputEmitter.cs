...
namespace Game.Application
{
    ...
    public sealed class InputEmitter : ICommandEmitter
    {
        ...
        public InputEmitter(IAsyncCommandQueue<GameCommand> queue, IInputService inputService)
        {
            _queue = queue;
        ...
        private async UniTaskVoid PumpAsync(CancellationToken ct)
        {
            while (!ct.IsCancellationRequested)
            {
                ...
                {
                    _queue.Enqueue(new GameCommand(GameCommandType.Pause));
                }

                if (_inputService.GetAddScoreInput())
                {
                    _queue.Enqueue(new GameCommand(GameCommandType.AddScore));
                }
            }
        }
    }
